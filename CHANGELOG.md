...
- **agent 行为评测在改版后逐项不变**（`python -m evals.run_eval`，规则模式，40.3 s）：
  ...
  已按新口径重生成入库。
- 测试规模 **247 → 277 项**（新增 `tests/test_price_calendar.py` 29 项：331 号文逐条窗口断言、
...
- 本机全量 **277 项全绿**（832 s，Python 3.13.9 / Anaconda）、快测 244 项 75 ~ 93 s，
  但 **1.1.0 还没走 CI**：CI 才是本仓库的发版标准口径。推上去后要 `workflow_dispatch`
  跑一次 `full-tests`，把 Run 编号与计时回填 `requirements.lock` 的「实测结果」，
  然后才打 `v1.1.0` 标签。
- 快测从上一版记录的 57 s 涨到 75 ~ 93 s，**本轮只登记现象、没有归因**：耗时集中在
  `tests/test_chat_agent.py` 那个真跑一次调度（65.3 s，MILP 占满）的 module 级 fixture，
  新口径电价曲线是否让分支搜索变长需要拿旧数据复测才能确认。
