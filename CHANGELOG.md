...
### 验证（Release gate）—— CI 全量已跑通（Run 35486570178，`c4d220c`）

- **CI 才是本仓库的发版标准口径**，两个 job 都在 `c4d220c` 上绿了：
  `fast-tests` 244 项 `in 50.45 s`（push 触发 Run 35478561322，job 含装依赖 92 s）、
  `full-tests` 277 项全绿（job 851 s，其中 `coverage run -m pytest -m ""` 一步 805 s，
  覆盖率 82%）。本机自查同步为快测 75 ~ 93 s、全量 832 s。
- **顺带查出一件小事：CI 的这一步从来不打印"N passed in Xs"**。命令里写了 `-q`，
  而 `pytest.ini` 的 `addopts` 已经带了一个 `-q`，两个叠加就是 `-qq`，pytest 把汇总行一起吞了
  ——上面那个 277 是数四行进度的圆点（72×3 + 61）数出来的，不是读来的。
  同一个坑也让本机一度以为"pytest 不打印汇总是环境问题"。已从 `ci.yml` 去掉重复的 `-q`；
  **这条修复还没推**，所以下一次 dispatch 起 CI 才会自己报计数与计时。
