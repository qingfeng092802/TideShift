...
一次**口径改版**：把"广东峰谷电价"从五套互相矛盾的自拟时段（含前端下载模板里内联的一套），换成有正式文件出处的
粤发改价格〔2021〕331 号划分，并加上缺失的月份维度。所有量化结论随之重测。
SemVer 取 minor：新增 `data/README.md` 溯源与 `tests/test_price_calendar.py`，
且求解缓存因 `PRICE_RULE_VERSION` +1 而整体失效（改了代码数字一定会变）。

### 变更（Changed）—— 电价日历按 331 号文重定

- **时段划分收敛到单一事实来源** `src/data/data_generator.py::PRICE_PERIODS`。此前全仓有
  **五套**口径：数据生成器、上传适配、后端 `params` 默认值、后端 `price_cards` 展示文案、
  以及前端模板下载里内联的一份（`h >= 17 && h < 22 ? 1.35 : ...`）。后两套是纯展示，
  但用户在设置页看到的时段与引擎真正按哪档算钱对不上，而前端模板是**用户下载后再上传回来**
  的数据源——等于用一套私有日历参与算钱。
- **加进月份维度**：低谷 0–8、高峰 10–12 / 14–19、平段为其余，尖峰 11–12 / 15–17
  且**只在 7、8、9 月执行**（文件原文"三个整月"，上浮 25%）。旧实现让尖峰全年生效，
  还把 19–21 时也算成尖峰。
- **四档价格改为按文件比例推导**，不再是手填字面量：平段基准 0.65 → 高峰 1.105（×1.7）、
  低谷 0.247（×0.38）、尖峰 1.38125（高峰 ×1.25）。尖峰刻意不取整，取整后比例一致性就破了。
- **`price_by_hour` 改为关键字参数 + 最窄窗口胜 + 落空即抛错**：
  位置参数会让 `price_by_hour(h, cfg)` 静默把配置当月份；`for + break` 的第一个命中即返回
  会让"把尖峰挪到高峰前后"悄悄换档；原来落不到任何区间时**默认返回平段价**，时段表改漏了
  不会报错，只会让一部分小时悄悄变成平段。
- **`PRICE_RULE_VERSION` 1 → 3**，并把注释里指向 `upload_adapter.PRICE_PERIODS` 的错路径
  改成实际定义处。不 +1 的话磁盘上旧缓存会顶着旧日历的价格被命中，症状是"改了代码数字没变"。
- **DR 默认事件与内置 DR 生成的时段跟着改**：默认第二事件 19:30–20:30 → **11:00–12:00**
  （旧窗口是按"尖峰 19–21"选的，在新口径下 19 点后是平段，削峰事件落在平段语义不成立）；
  `generate_dr_signals` 的候选整点从 `[10,11,14,15,16,19,20]` 改成 `[10,11,14,15,16,17,18]`。
- **设置页电价输入 `step` 0.01 → 0.00001**：档位价现在有 5 位小数，`step=0.01` 会让
  `1.38125` 处于非法步进态；电价卡片的显示也不再 `toFixed(2)`（会把 1.38125 显示成 1.38）。
- **`data/` 重新生成**（`seed=42`）：`load_kw` 与 `ambient_temp_c` 逐字节未变，
  电价列换成新四档，`dr_signals.csv` 两个事件随之移动。新增 `data/README.md`：
  文件原文与本仓库实现的逐条对应表、推导链、以及"0.65 是示例基准价不是到户价"这类限制。

### 修复（Fixed）—— 两处"输入被静默替换"的错误，以及它们的错误归因

- **`StorageOptimizationAgent.optimize()` 第二位参数是一个从未被读取的 `load_profile`**，
  于是全仓 11 处 `optimize(PRICE, AMB, ...)` 位置调用把**气温曲线绑到了它上面**：
  气温被丢弃、退化成常量 30 ℃，热约束随之放松。删除该参数，第二位现在是真正的
  `ambient_temp_profile`；生产链路的三个调用点本来就按关键字传参，行为不变。
  受影响的是所有 `optimize(price, amb)` 形式的历史测量与测试断言。
- **寻优 Agent 与评测 harness 自己现场抽气温**：`generate_ambient_temp` 内含一行
  **未播种**的 `np.random.normal(0, 0.8, n)`，每新建一个 agent 就是另一天天气。
  改为读当日落盘序列（`data_loader.day_price_temp()`）。改完之后
  `ParameterSearchAgent.evaluate({}, 96)` 与直接调 `optimize()` 得到同一个数
  （**1373.58 元**），两条路径首次逐分对齐。
- **因此，"同一配置重复求解实测能差约 1%，所以胜出门槛取 2%"这条论证是错的归因**：
  差的不是解，是输入。修输入之前，24 点默认约束连跑四次落在 **1352.18 ~ 1392.01 元，
  极差 2.9%——比门槛本身还大**；修完之后同输入重复求解**逐分不差**。
  门槛保持 2%，但理由改为"盖住 `mip_gap=1%` 并留一倍余量"。
  主 README 护栏 4、`docs/experiments.md` §1、`src/agents/parameter_search_agent.py`
  的注释与报告文案同步改写。
- **一个曾被写成"诚实空结论"的数字其实出自错误输入**：旧结论"SOC 区间加权相对常数近似
  当天只多 1.67 元（0.14%）"，是在气温被换成常量 30 ℃ 的调用上测的。修正后重测为
  **1373.58 vs 1346.23 元，+27.35 元（+2.03%）**（常数模型给自己那条曲线计衰减 557.81 元、
  后验真值 384.30 元，高估 45.1%，于是让出 30.55 元套利而真实衰减几乎没省）。
- **`gap 容差换收益`的旧结论也是反例**：同一输入把 `mip_gap` 从 1% 收到 0%，
  39.1 s → **303.3 s**（7.8 倍），目标函数值只动 +0.67 元，**报表净收益反而少 31.79 元
  （−2.31%）**。根因是两套衰减计费不同源（同一条曲线目标内 382.54、后验 420.29，
  差 37.75 元 / 9.0%）。已作为**已知局限**写进中英 README，而不是藏起来。
- **CSV 与寻优报告的写文件锁 LF**（`lineterminator` / `newline`）：pandas 与 `open()` 在
  Windows 默认按 `os.linesep` 写，生成物于是与 `.gitattributes` 钉的 `* text=auto eol=lf`
  相反——入库时被静默归一化，但工作树字节和新克隆的不一致。
- **防幻觉探针埋的"编造数字"在新电价下不再是编造**：`gp-fabricated-money` 埋的是
  `4521.00 元` 与 `0.87 元/kWh`，而 331 号文口径下事实摘要里出现了 `0.88`（窗口均价按两位
  小数印出）。`check_grounding` 的匹配式是 `|v − t| ≤ 0.01·|t| + 0.05`，0.87 与 0.88 相差 0.02
  → 守卫判它"有依据"并放行，`grounding_catch_rate` 于是从 0.750 掉到 0.500，
  `tests/test_evals_run.py::test_no_regression_against_baseline` 报红。
  **指标掉的是探针效度，不是守卫能力**——守卫的行为完全正确（0.87 确实在容差带内可验算）。
  处理：埋值改为 `1.62 元/kWh`（离任何真实值都在容差带外），捕获率回到 0.750；
  并新增 `tests/test_evals_run.py::test_planted_numbers_are_not_accidentally_grounded`
  把"埋的数必须真的不在摘要里"钉成断言，以后口径再变导致相撞会**点名相撞的真实值**，
  而不是让指标悄悄漂移。顺带记录守卫的固有盲区：元/kWh 量级上有一条 **±0.05 元（约 6%）免疫带**，
  而这个 0.05 是摘要自身两位小数取整（`1.105 → 1.1`、`0.247 → 0.25`）逼出来的，
  收紧就会把真数字判成编造——已写进中英 README 的已知局限。

### 验证（Verified）—— 改版后全部量化数字重测（Python 3.13.9 / Anaconda，2026-09-20）

调度日 2024-07-30，整轮含页面默认两个 DR 事件：

| 指标 | 旧电价口径 | 新电价口径 |
|------|-----------|-----------|
| 优化净收益（套利 − 衰减） | 1198.12 元 | **1427.90 元** |
| 基准净收益 | 626.49 元 | 711.71 元 |
| 优化 vs 基准 | +91.2% | **+100.6%** |
| 热约束的代价 | −8.5%（110.88 元） | **−3.3%（48.54 元）** |
| 同口径年化 | 43.7 万元 | **52.1 万元** |
| 含 DR 净收益 | 1731.89 元 | 1587.74 元 |

- ⚠️ **年化变大的原因要说清楚**：不是"政策变了储能更赚钱"，而是本仓库旧实现自拟的那组
  比例本来就偏保守。文件给的峰:平:谷 = **1.7 : 1 : 0.38**，旧字面量折算下来是
  **1.615 : 1 : 0.492**——低谷被定贵了（0.32 → 0.247）、高峰被定便宜了（1.05 → 1.105），
  于是每 kWh 的谷充峰放价差从 0.730 元拉开到 **0.858 元（+17.5%）**；再叠加低谷窗口
  从 7 小时增到 8 小时。唯一让收益变小的方向是尖峰：从全年 5 小时/天缩到
  **7/8/9 月共 3 小时/天**，而套利主要靠高峰与低谷那一档，所以净效果是变大。
  这个数字必须连同口径与比例出处一起引用。
- 无热约束那一列本轮 DR 补贴为 **0 元**（两个默认事件都被 DR Agent 拒绝：预测温度 72.3 ℃、
  SOC 不足）——这是"关掉约束"的代价之一，已写进表注。
- 默认调度日 2024-07-15 重测：含 DR 净收益 1567.82 元、最高温 47.08 ℃、整轮 59.1 s。
- **agent 行为评测在改版后逐项不变**（`python -m evals.run_eval`，规则模式，40.3 s）：
  32 例、任务成功率 0.969、数字保真 1.000、副作用守卫 1.000、编造捕获 0.750——
  与 `evals/reports/baseline.json` 逐位一致，所以**基线没有迁移**。这些指标量的是
  "agent 有没有把任务做对、数字是不是抄的"，与电价口径无关；会变的只有失败的那一条
  用例（`q-baseline-temp`，改版前后同一条）。`evals/reports/rule-latest.{md,json}`
  已按新口径重生成入库。
- 测试规模 **247 → 277 项**（快测 215 → 244、`slow` 32 → 33；新增 `tests/test_price_calendar.py` 29 项：331 号文逐条窗口断言、
  比例一致性、顺序无关性、关键字参数防误绑、两条路径同源、"后端不许再抄一份时段文案"与
  "前端模板不许再内联一套日历"的守卫，以及本轮那两个输入错误的回归锁：`optimize()` 第二位
  必须是气温、寻优准备输入时不得动用全局随机流）。
- 界面截图与演示 GIF **仍是改版前口径**，已在 README 两处标注，重录待办。

### 验证（Release gate）—— CI 全量已跑通（Run 35496788350，`70ea9ae`）

- **CI 才是本仓库的发版标准口径**，`v1.1.0` 标签打在被 CI 验证过的那个 commit（`70ea9ae`）上：
  `full-tests` **`277 passed, 1 warning in 796.12 s`**（job 整跑 836 s，覆盖率 82% =
  6650 stmts / 1217 miss）、`fast-tests` **`244 passed, 33 deselected in 50.45 s`**
  （push 触发 Run 35478561322，job 含装依赖 92 s）。本机自查同步为快测 68 ~ 93 s、全量 832 s。
  引用全量计计时用 pytest 自报的 796 s（纯测试）而不是 job 的 836 s（含 checkout 与装依赖）。
- **顺带查出一件小事：CI 的这一步从来不打印"N passed in Xs"**。命令里写了 `-q`，
  而 `pytest.ini` 的 `addopts` 已经带了一个 `-q`，两个叠加就是 `-qq`，pytest 把汇总行一起吞了
  ——第一次跑这道门槛（Run 35486570178）时那个 277 是数四行进度的圆点（72×3 + 61）数出来的，
  不是读来的。同一个坑也让本机一度以为"pytest 不打印汇总是沙箱问题"，其实上面去掉一个 `-q`
  就正常打印。已从 `ci.yml` 去掉重复的 `-q` 并复跑验证，于是有了上一条那个自报数字。
- 快测从上一版记录的 57 s 涨到 68 ~ 93 s（三次实测 67.7 / 75.5 / 92.8 s），**本轮只登记现象、没有归因**：耗时集中在
  `tests/test_chat_agent.py` 那个真跑一次调度（65.3 s，MILP 占满）的 module 级 fixture，
  新口径电价曲线是否让分支搜索变长需要拿旧数据复测才能确认（CI 上同一套是 50.45 s）。
...
- `src/agents/llm_explainer.py` 的「为什么这么设计（**面试**可讲的三条）」去掉括号——
  三条设计理由是正当内容，那句备注不是。
- 遗留：`backend/server.py` 仍有 **1730 行**（含 29 个端点与会话管理）。已按注释清理降低可读性
...
### 新增（Added）—— 2026-09-19 看板运行追溯页

- **侧栏「运行追溯」页**：trace 层此前只有接口，README 的"已知未完成项"原话是
  "看板没有追溯页，`/api/traces` 只能 curl"。现在补上只读视图：最近运行列表
  （整轮耗时 / 环节数 / 异常数 / 最慢环节）+ 点一行展开单次运行的事件序列（每环节一条占比条）。
  原生 JS + 现有 CSS 变量，零构建步骤、断网可用，浅色主题下跟随主题色。
- **`/api/traces` 一并回传自己的口径**：新增 `trace.status()` → `{level, capacity, kept}`，
  页面据此解释"为什么这里看不到问题原文"（`basic` 级只记长度）与"为什么只有最近 N 次"。
  前端不再复制一份默认值；**日志目录绝对路径不回传**（对本机布局没用的信息对侦察者有用）。
- **求解过程进 trace**：`forecast` 与 `milp_optimize` 两个环节在两种编排引擎里用同一组环节名
  记录。否则页面上"调度求解 50.97 s"之后是一片空白——只有一个总数等于什么都没解释。
  本机真实一次：`forecast 3.39 s` + `milp_optimize 47.47 s`。
- 新测试 5 项（`test_trace.py` 18 → 22、`test_server_api.py` +1）：口径回传、口径 env 写坏时的兜底、
  断线落终态、NAV/RENDERERS/`<section>` 三处接线一致、SSE 运行必须在生成器内部开 run。
  测试规模 **233 → 238**（快测 201 → 206）。

### 修复（Fixed）—— 追溯页上线当天抓到的两个口径错误

- **`/api/chat/stream` 的运行记录是假的**：端点用 `@trace.traced` 装饰，而装饰器的 `with`
  只圈到"生成器对象被创建"就退出，真正的 LLM 与工具调用发生在之后被逐块消费时——记出来是一条
  **亚毫秒、0 环节、属性全空**的运行。追溯页把这条列出来时一眼就不像话，于是顺着查到了根因。
  现在 run 开在 `gen()` 内部，属性（`mode` / `date` / `stream` / `question_len`）在真实消费后才落账。
- **断线的 SSE 会留下"进行中"的僵尸运行**：客户端提前断开时生成器收到的是
  `GeneratorExit`（`BaseException` 分支，不被原来的 `except Exception` 接住），
  `trace.run` 于是既不写终态也不报错，页面上那条运行永远显示"进行中"。
...
- `LICENSE` 与两份 README 的版权署名由真实姓名改为 GitHub handle `qingfeng092802`；随后用
  ...
  重写过程一度丢掉本地 `refs/remotes/origin/main`，当时据此判断"远端仍是重写前历史、
  必须 `push --force` 才能跟上"。**本轮用 `git ls-remote origin` 只读核对，该判断已过期**：
  远端 `main` 已在重写后的链上（`1f12325`，本地 reflog 记为 2026-09-19 13:14 的一次推送），
  `v1.0.0` 两侧同指 `5a59f04`，本地提交以 fast-forward 方式领先，普通 `git push` 即可跟上。
  本条目自身未做任何远端操作。

### 变更（Changed）—— 2026-09-19 `full-tests` 不再把整套测试跑两遍

- `pytest -m "" -q` 与 `coverage run -m pytest -m "" -q` 合并为一句。实测两步各 846 / 844 s，
  job 总时长 1732 s 里近一半是把 247 项重跑一遍；覆盖率本就在一次运行里插桩采集，第二遍
  不产生任何新证据。**代价是计时口径变了**：合并后那一步含 coverage 开销，文档里引用的
  846 s 因此被明确标注为"合并前那次运行的单步实测"，而不是当前布局的预测值。
  改动只在本地提交，需推送后下一次 nightly / 手动触发才会生效。

### 验证（Verified）—— 2026-09-19 v1.0.0 的 CI 全量门槛（补账）

- 用 `workflow_dispatch` 在 `e3df672` 上跑通 **`full-tests`：247 项全绿**（Run 35446508669，
  `ubuntu-latest` / Python 3.13，`pytest -m ""` 单步 **846 s**）。
  此前"发版门槛 = 手动触发的 CI 全量通过"这条**在 v1.0.0 上从未成立**：仓库里最近一次全量是
  09-16 的 `a087bc2`（重写前链、且早于 trace 层 / 评测层 / 前端用例），`e3df672` 上只有
  push 事件的快测。Release 已经先发了，这一步属于**补账**，如实记录而不是回头改文案。
- 顺带量出一处重复劳动：`full-tests` 把整套测试**跑了两遍**（`pytest -m "" -q` 846 s +
  `coverage run -m pytest -m "" -q` 844 s），job 总时长 1732 s 里约 49% 是重跑。
  合并成一句 `coverage run -m pytest -m "" -q && coverage report` 可省掉约 14 分钟，
  这属于 CI 基础设施变更，所以先只记录不动手；经确认后已合并（见下方「变更」条目）。
- `requirements.lock` 的「实测结果」按该文件第 3 条维护约定回填为上面的真实数字。

### 修复（Fixed）—— 2026-09-19 全仓口径审计（一次通读，逐条对物证）

把仓库里所有"数字型说法"过了一遍，只留实测与物证对得上的，其余当场更正或登记为未复核：

- **规则模式应答延迟 `0 / 0 ms` → `0.1 / 1.3 ms`**：表里抄的是早期取整前的 0，而
  `evals/reports/rule-latest.md` 记的是 0.1 / 1.3。指标表里的 0 会被读成"根本没有延迟"。
- **看板页面数 `6` → `7`**：追溯页是本轮自己加的，功能表与页面清单都没跟上——
  正是 `tests/test_trace.py::test_nav_pages_are_wired_end_to_end` 能查出口径不一致的那类漏更。
- **`load_dr_signals()` "仅被测试引用"说法有误**：命令行寻优脚本 `--with-dr` 也在调它。
  结论（不在 Web 链路上）不变，但引用范围不能写错。
- **`src/utils/config.py` 热参数注释仍指向 `docs/PARAMETERS.md`**——该文件从未入库，
  是"修掉 20 余处悬空引用"那一轮漏掉的第 21 处；改为注释内自解释。
- **参数寻优的求解器抖动三元组被抄串**：`parameter_search_agent.py` 里写成
  `1244.24 / 1255.87 / 1285.79`，第三值是跨分辨率那组数的 24 点项。已按
  `docs/parameter-search-sample.md`（默认配置 24 点 = 1260.58）更正，并把来源写进注释——
  `MATERIAL_GAIN_RATIO = 2%` 的论证全靠这组抖动量级。
- **README 里一处未填的占位**："退化 X 个百分点"改成报告实值 5.4。
- **求解耗时上界 `43 s` → `51 s`**：文档自己引用的那条 trace 就是 50.97 s，
  `logs/app.log` 另有 49.7 / 51.0 s 两行，旧上界被自家物证反驳。
- **参数寻优样本 `1.7 s` → `1.8 s`**：以样本文件本身为准（`docs/parameter-search-sample.md`）。
- `requirements.lock` 顶部"实测结果"仍写快测 79 / 全量 102，按该文件自己第 3 条维护约定更新。
- 测试文件表补上漏掉的两行（`test_review_fixes.py` 10 项、`test_dr_thermal_overshoot.py` 2 项），
  目录树的 `evals/reports/` 说明补上 `llm-latest.md`。
- **登记一条未能解决的矛盾**（写入 `docs/experiments.md` §4，并在两份 README 就地标注"未复核"）：
  跨分辨率三元组里的 24 点值 1285.79 元，与寻优样本记录中默认配置 24 点的 1260.58 元差 2.0%。
  两者不可能同时成立，而 2% 又恰好等于胜出门槛，所以这条要么重跑对齐、要么改掉论证引用。
  **没有为了自洽去改任何一个数**。
- 历史条目不追溯改写：早期那条 `tests/test_parameter_search.py（49 项）` 与当前 52 项
  不符，是后续增量没记账。按 Keep a Changelog 的约定留在原处，只在这里说明。

### 新增（Added）—— 2026-09-19 量化成果表整轮复现

- 主 README「量化成果」的 MILP 稳态那一列**重新复现通过**，复现脚本与判据写进
  `docs/experiments.md` §1（含 DR 两事件、`use_ml_forecast=True`，整轮 42.7 s：
  套利 1702.79 / 衰减 504.67 / 含 DR 净收益 1731.89 / 最高温 46.99 / 等效循环 1.8925）。
  判据是**后验衰减成本**而不是套利收益——套利那一列对"整轮 vs 单次 MILP"不敏感，
  拿它对口径会假装复现成功。
- **顺带查出一个错误标注**：量化表"求解时间"原写 `MIP gap = 0%`，实测这些数字出自代码默认的 1%；
  同一输入收到 gap 0% 需 100.6 s 且解出另一个更优基础调度（1219.75 → 1254.59，+2.9%）。
  即 **gap 容差的代价可以大于容差本身**：目标函数按 in-objective 衰减成本计费、报表按后验
  区间加权重算，两套口径不同源时 1% 会放大成 2.9% 的结果差。
- **把"区间加权衰减建模"的价值从一句夸大改成实测**：旧文案"精确模型 1245.30 vs 常数近似
  1224.08"两个数都属已被本仓库更正掉的 2026-09-07 口径（改表时漏改这句括号）。重测结果
  1219.75 vs 1218.08，差 **+1.67 元（0.14%）**，不到 2% 门槛、也在 1% gap 噪声内；
  常数系数把衰减成本高估 42%（556.45 vs 真实 391.49），优化器因此少赚 93.22 元套利、
  少耗 91.55 元真实衰减。文案改为"它改变的是优化器看见的成本结构与取舍，不是当天报表"。
