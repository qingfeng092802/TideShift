# -*- coding: utf-8 -*-
"""🟠#42/T4：数值边界测试（衰减模型除零 / NaN SOC / 热模型末点统计）
...
def test_estimate_degradation_zero_dod_raises():
    """🟠#42：soc_range 上下限相等（DoD=0）必须显式报错而非除零/inf。"""
...
def test_degradation_clip_uses_config_bounds():
    """🟠#15：衰减模型重放 SOC 按 soc_min/soc_max 裁剪（而非 0~1）。"""
    m = BatteryDegradationModel(CONFIG.battery)
...
def test_estimate_temperature_rise_charge_sign():
    """🟠#28：estimate_temperature_rise 充电/放电符号必须与 compute_heat_generation 自洽。
    ...
    assert t_chg < t_dis, f"充电稳态温升({t_chg:.2f})应低于放电({t_dis:.2f})（🟠#28 符号不自洽）"
