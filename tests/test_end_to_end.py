...
def test_temperature_within_safe_limit(report):
    ...
    ⚠️ 为什么不再用 47.5℃ 这类"某次实测值"当阈值：
      ...
      331 号文电价口径下重测（2026-09-20）两个调度日的后验峰值是 46.92℃（2024-07-30）
      与 47.08℃（2024-07-15）：都还在 47.5℃ 之下，但离它只剩 0.4℃。换一套时段划分
      就把 0.5℃ 的裕度改成 0.4℃——求解器小版本变化、MIP gap 内多解、后验 RC 仿真的
      非线性同样都会让该数字浮动，这类断言会变成随机失败源，而不是质量门。
