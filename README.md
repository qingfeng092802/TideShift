...
- **Agent 行为是被度量的**：32 条用例 → 工具调用成功率 **0.969**、数字保真 **1.000**、编造数字捕获率 **0.750**，
  连统计口径和已知盲区一起写在 [`evals/`](evals/README.md)；
- **会自己调参，也如实认输**：提案 → 真实 MILP 评价 → 同分辨率复验的闭环；这组数据上**没跑赢默认约束**，
  [报告](docs/parameter-search-sample.md)里就写"未跑赢"；
- **可复现**：**232 项测试**（200 快测 + 32 slow）、`requirements.lock` 锁依赖、前端资源本地分发，**断网可跑全流程**。
